...
module Generator =
    ...
    let mkRouteDoc (route : Analyzer.RouteInfos) : RouteDescriptor =
        let responses =
            ...
            |> List.map (
                fun rs ->
                    let schema =
                        if rs.ModelType.IsSwaggerPrimitive
                        then None
                        else Some (rs.ModelType.Describes())
                    let model =
                        {
                            Description = sprintf "code %d returns %s" rs.StatusCode rs.ContentType
                            Schema = schema
                        }
                    (rs.StatusCode, model))
            |> dict
    ...
    type DocumentationAddendumProvider =
        Analyzer.RouteInfos
            -> string * HttpVerb * PathDefinition
            -> string * HttpVerb * PathDefinition

    let DefaultDocumentationAddendumProvider = fun _ doc -> doc

    let convertRouteInfos
        (route      : Analyzer.RouteInfos)
        (addendums  : DocumentationAddendumProvider)
        : (string * HttpVerb * PathDefinition) =
        ...
        let operationId =
            if route.MetaData.ContainsKey "operationId"
            then route.MetaData.["operationId"]
            else ""
        let consumes =
