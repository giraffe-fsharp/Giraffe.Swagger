    ...
    let handle (next : HttpFunc) (ctx : HttpContext) =
        ...
        | Some ze ->
            let mimetype =
                ...
                | ".js"   -> "text/javascript"
                | ext when
                    [ ".gif"; ".png"; ".jpeg"; ".jpg"; ".bmp"; ".webp" ]
                    |> List.contains ext -> sprintf "image/%s" ext
                | ext when
                    ext.StartsWith "." -> "application/" + ext.Substring(1)
                | _ -> "application/octet-stream"

            ctx.Response.ContentType <- mimetype
            use r = new StreamReader(ze.Open())
            let bytes =
                r.ReadToEnd().Replace("http://petstore.swagger.io/v2/swagger.json", (combineUrls "/" swJsonPath))
