...
module Analyzer =
    ...
    type FormatParsed =
        | StringPart  | CharPart    | BoolPart
        | IntPart     | DecimalPart | HexaPart
        ...
        member __.AddRoute verb parameters path =
            ...
            ctx.CurrentRoute :=
                Some {  Verb        = verb
                        Path        = path
                        Responses   = []
                        Parameters  = ( __.Parameters @ parameters)
                        MetaData    = Map.empty }
            ctx
    ...
    let analyze webapp (rules : AppAnalyzeRules) : AnalyzeContext =
        ...
        let rec loop exp (ctx : AnalyzeContext) : AnalyzeContext =
            ...
            match exp with
            | Value (o, _) -> ctx.AddArgType (o.GetType())
            | Let (v, NewUnionCase (_, handlers), Lambda (__, Call (None, m, _)))
                when v.Name = "handlers" && m.Name = "choose" && m.DeclaringType.Name = "Core" ->
            ...
            | Let (id, op, t) ->
                ...
                | Call (None, method, __) when method.Name = "TypeOf" ->
            ...
            | Application (Application (PropertyGet (None, op, _), PropertyGet (None, (IsHttpVerb verb), _)), exp)
                when op.Name = "op_GreaterEqualsGreater" ->
                let v = Some (verb.ToString())
                let ctx = { ctx with Verb = v }
                loop exp ctx

            | Application (Application (PropertyGet (None, op, []), Let (varname, Value (name,_), Lambda (_, Call (None, method, _)))), exp2)
                when op.Name = "op_EqualsEqualsGreater" ->
            ...
            | Application (Application (PropertyGet (None, op, []), exp1), ValueWithName _)
                when op.Name = "op_GreaterEqualsGreater" ->
            ...
            | Call(__, method, args) when method.Name = "choose" && method.DeclaringType.Name = "Core" ->
